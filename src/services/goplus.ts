...
const parseTokenSecurityDataRaw = (
  data: TokenSecurityDataRaw,
): TokenSecurityData => {
  return {
    ...data,
    ...
    can_take_back_ownership: stringToBooleanOptional(
...
const isSafeToken = (
  ...
  if (
    data.is_airdrop_scam === true ||
    data.is_true_token === false ||
...
const generateTokenSecurity = async (
  ...
  if (tokenAddress === AddressZero) {
    return {
      ...
      explorerUrl: `${
        getChainById(chainId).metamask.blockExplorerUrls[0]
  ...
  const tokenSecurity = {
    ...
    goplusUrl: data
      ? `https://gopluslabs.io/token-security/${chainId}/${tokenAddress}`
      : undefined,
  };
