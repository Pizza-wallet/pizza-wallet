...
const chainsTokensToolsContext =
  createContext<ChainsTokensToolsContextProps>(initialData);
...
export const ChainsTokensToolsProvider = ({ children }: AuxProps) => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!chains) {
        ...
        console.warn(
          "possibilities request did not contain required setup information",
        );
        return;
      }
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!tokens) {
        // eslint-disable-next-line
        console.warn(
          "token request did not contain required setup information",
        );
        return;
      }
      ...
      Object.keys(oldTokens).forEach((chainKey) => {
        oldTokens[chainKey].forEach((token) => {
          ...
          if (
            !newTokens[chainKey].find((item) => item.address === token.address)
          ) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (!tools.bridges || !tools.exchanges) {
        // eslint-disable-next-line
        console.warn(
          "tools request did not contain required setup information",
        );
        return;
      }
      // bridges
  ...
  return (
    <chainsTokensToolsContext.Provider value={data}>
      {children}
    </chainsTokensToolsContext.Provider>
  );
};
