...
interface TokenSelectProps {
  ...
  onChangeSelectedToken: (tokenAddress: string) => void;
...
const TokenSelect = ({
  ...
  const token =
    selectedToken && selectedChain
      ? findToken(selectedChain, selectedToken)
      : undefined;

  const [tokenSecurity, setTokenSecurity] = useState<TokenSecurity>();
  ...
  useEffect(() => {
    if (!token) {
      ...
      goplus
        .getTokenSecurity(token.chainId, token.address)
        .then(setTokenSecurity);
    }
  }, [token]);
  ...
  function getTooltip() {
    ...
    switch (tokenSecurity.state) {
      case TokenSecurityState.SAFE:
        return (
          <>
            ...
            <a
              href={tokenSecurity.explorerUrl}
              target="_blank"
              rel="nofollow noreferrer"
            >
              View In Explorer
      ...
      case TokenSecurityState.ATTENTION:
        return (
          <>
            {tokenSecurity.state === TokenSecurityState.RISKY
              ? "Risky Token"
              : "Verify Token"}
            <br />
            {tokenSecurity.goplusUrl && (
      ...
      >
        ...
        {selectedChain && tokens[selectedChain]?.length > 6 && (
          <Select.OptGroup label="Owned Token">
            {/* TODO: Check user is connected via web3 if not show button to connect */}
            {balances &&
              balances[selectedChain] &&
              balances[selectedChain]?.length === 0 && (
            ...
            {balances &&
              ...
              tokens[selectedChain]
                .filter((token) => token.amount?.gt(0)) // TODO: check why this isnt working
                .map((token) => (
                  <Select.Option
